...
if (sessionStorage.getItem("cart-number") == 0) {
	...
	for (let i = 0; i < sessions.length; i++) {
		if (
			sessionStorage.getItem(sessions[i]) != 0 &&
			sessions[i].match("-countprod") != null
		) {
			var node = document.createElement("li");
	...
	for (let i = 0; i < sessions.length; i++) {
		if (sessions[i].match("-price") != null) {
			total +=
				parseInt(sessionStorage.getItem(sessions[i])) *
				parseInt(
					sessionStorage.getItem(sessions[i].slice(0, -6) + "-countprod")
				);
		}
	}
			...
			if (
				...
				fetches.push(
					fetch("../../private/addorder.php", {
						...
						body:
							"p=" +
							sessions[i].slice(0, -10) +
							"&t=" +
							sessionStorage.getItem(sessions[i]),
					})
				);
			}
		}
